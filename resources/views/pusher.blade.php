...
<body class="bg-gray-100 flex items-center justify-center h-screen">
    ...
    <div class="w-full max-w-lg bg-white p-6 rounded-lg shadow-lg">
        ...
        <textarea id="message" class="w-full mt-4 p-2 border border-gray-300 rounded-lg" rows="2"
            placeholder="Type a message..."></textarea>
        <button onclick="sendMessage()"
            class="w-full mt-2 bg-blue-500 text-white py-2 rounded-lg hover:bg-blue-600">Send</button>
    </div>
    ...
    <script>
        ...
        const token = "{{ session('myapitoken') }}";
        const pusher = new Pusher("{{ env('PUSHER_APP_KEY') }}", {
            cluster: "{{ env('PUSHER_APP_CLUSTER') }}",
            encrypted: true,
            authEndpoint: "{{ url('/broadcasting/auth') }}", // Specify the auth endpoint for private channels
            auth: {
                headers: {
